  ...
  filterAndRenderItems() {
    ...
    if (filterValue && filterValue !== '') {
      filtered = filtered.filter((item) => {
        ...
        switch (filterValue) {
        case 'VR': {
          const isVR = name.includes('vr')
                       || name.includes('180x180')
                       || name.includes('360');
          return (type === 'mp4' || type === 'video') && isVR;
        }
        case 'Video': {
          const isNotVR = !name.includes('vr')
                          && !name.includes('180x180')
                          && !name.includes('360');
          return (type === 'mp4' || type === 'video') && isNotVR;
        }
        case 'Images': {
          const isImage = ['jpg', 'jpeg', 'png', 'webp', 'image'].includes(type);
          return isImage;
        }
        case 'Archive': {
          const isArchive = ['zip', 'rar', '7z'].includes(type);
          return isArchive;
        }
        case 'Folder': {
