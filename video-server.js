...
videoRouter.get('/direct', (req, res) => {
    try {
        ...
        console.log('[Video Server] Headers:', req.headers);
        
        let cleanPath;
        try {
            // Get path from query and decode it
            const videoPath = decodeURIComponent(req.query.path);
            ...
            console.log('[Video Server] Checking if file exists:', cleanPath);
            const exists = fs.existsSync(cleanPath);
            console.log('[Video Server] File exists:', exists);
            ...
            try {
                ...
                console.error('[Video Server] Access error:', accessError);
                res.setHeader('Accept-Ranges', 'bytes');
                res.setHeader('Content-Type', 'video/mp4');
                return res.status(403).json({
                    error: 'Cannot read file',
